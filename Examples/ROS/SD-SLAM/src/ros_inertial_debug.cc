...
int main(int argc, char **argv) {
  ...
  if(argc < 2 and argc > 3) {
    cerr << endl << "Usage: rosrun SD-SLAM Inertial path_to_settings" << endl;
    ros::shutdown();
    return 1;
  }
  ...
  Madgwick global_mad = Madgwick(gain);

  double last_t = 0.0, dt = 0.0;
  ROSPublisher pub = ROSPublisher(n);
  SLAM.GetTracker()->set_debug = true;
  ...
  while (ros::ok() && !SLAM.StopRequested()) {
    if (reader.HasNewImage()) {
      ...
      pub.publish_orientation(pred_mm, time_now, "odom", "SLAM/local/Motion_Model");
      pub.publish_orientation(pred_mad, time_now, "odom", "SLAM/local/IMU");

      Quaterniond local_cam(pose.block<3,3>(0,0));
      Quaterniond local_imu = global_mad.get_local_orientation();
      pub.publish_orientation(local_cam, time_now, "odom", "SLAM/local/cam");
      pub.publish_orientation(local_imu, time_now, "odom", "IMU/local");

      // WORLD
      global_mad.update(imu.acceleration(), imu.angular_velocity(), dt);
      if (count == 150){
        global_mad.set_orientation_from_frame(pose);
      }
      count++;
      Quaterniond world_cam = SLAM.GetTracker()->madgwick_.get_orientation();
      Quaterniond world_imu_sdslam = camera_to_world(pose);
      Quaterniond world_imu = global_mad.get_orientation();
      pub.publish_orientation(world_cam, time_now, "odom", "SLAM/World/cam");
      pub.publish_orientation(world_imu_sdslam, time_now, "odom", "SLAM/World/IMU");
      pub.publish_orientation(world_imu, time_now, "odom", "IMU/World");
