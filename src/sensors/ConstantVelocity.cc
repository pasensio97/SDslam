...
Eigen::MatrixXd ConstantVelocity::jF(const Eigen::VectorXd &X, double time) {
  ...
  // Jacobian F
  // dx/dx   dq/dx   dv/dx   dw/dx       I     0     I*t     0
  // dx/dq   dq/dq   dv/dq   dw/dq       0     dq/dq 0       dw/dq
  // dx/dv   dq/dv   dv/dv   dw/dv   =   0     0     I       0
  // dx/dw   dq/dw   dv/dw   dw/dw       0     0     0       I
  jF.setIdentity();
  ...
  // dw/dq
  Eigen::Quaterniond qold(q(0), q(1), q(2), q(3));
  jF.block<4, 3>(3, 10) = dw_by_dq(qold, w, time);
...
Eigen::MatrixXd ConstantVelocity::Q(const Eigen::VectorXd &X, double time) {
  ...
  Eigen::Quaterniond qold(q(0), q(1), q(2), q(3));
  G.block<4, 3>(3, 3) = dw_by_dq(qold, w, time);
...
Eigen::MatrixXd ConstantVelocity::jH(const Eigen::VectorXd &X, double time) {
  ...
  // Jacobian H
  // dx/dx   dq/dx   dv/dx   dw/dx       I     0     0     0
  // dx/dq   dq/dq   dv/dq   dw/dq   =   0     I     0     0
  jH.setZero();
